...
VERSION   ?= $(call get_gh_version,nirui/sshwifty)
...
get_config: ## get default config from github
	curl \
		-o $(CURDIR)/root/defaults/sshwifty.conf.example.json \
		-SL https://raw.githubusercontent.com/nirui/sshwifty/refs/heads/master/sshwifty.conf.example.json
